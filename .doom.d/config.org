...
#+begin_src emacs-lisp
  ...
  (setq doom-variable-pitch-font (font-spec :family "ETBookOT" :weight 'regular))
#+end_src
* UX Improvements
...
#+end_src
** Python
*** Project Environments (PET)
The Python Executable Tracker
  ...
  (use-package! mu4e
    ...
    (when (f-exists?
      ...
      (add-to-list
       ...
       (make-mu4e-context
        :name "work"
        :vars `((user-mail-address . "correl@gisual.com")
                ...
                (mu4e-maildir-shortcuts . (("/Work/INBOX" . ?i)
                                           ...
                                           ("/Work/[Gmail]/Trash" . ?t)))
                (mu4e-compose-signature . ,(with-temp-buffer
                                             (when (f-exists? "~/.signature-gisual")
                                               (insert-file-contents "~/.signature-gisual"))
                                             (buffer-string)))
                (smtpmail-smtp-user . "correl@gisual.com")
                (smtpmail-smtp-server . "smtp.gmail.com")
                (smtpmail-smtp-service . 465)
                (smtpmail-stream-type . ssl)))))
    ...
    (setq mu4e-user-mail-address-list '("correl@gmail.com" "correl@gisual.com"))
...
*** News :news:
**** https://epgn.com/feed/
**** https://rss.beehiiv.com/feeds/ZwjrCnHCyS.xml
*** Fun :fun:
...
#+begin_src emacs-lisp
  (after! org
    (use-package jira-api
      :config (setq jira-api-host "jira.COMPANYNAME.io"
...
#+begin_src emacs-lisp
  (after! forge
    ;; Add repositories to forge-alist here
    )
#+end_src
** Eshell
...
#+begin_src emacs-lisp
  (use-package! prodigy
    ...
    (when (executable-find "imapnotify")
      ...
      (when (f-exists? (expand-file-name "~/.config/imap_inotify/work.js"))
        (prodigy-define-service
          ...
          :kill-signal 'sigkill))
      (when (f-directory? (expand-file-name "~/.config/imap_notify/personal.js"))
        (prodigy-define-service
    ...
    (when (f-exists? (expand-file-name "~/roam/personal/public"))
      (prodigy-define-service
        :name "Org Roam Documents (Personal)"
        :command "python"
        :args '("-m" "http.server" "3002")
        :cwd (expand-file-name "~/roam/personal/public")
        :tags '(personal autostart)
        :kill-signal 'sigkill))
    (when (f-exists? (expand-file-name "~/roam/gisual/public"))
      (prodigy-define-service
        :name "Org Roam Documents (Gisual)"
        :command "python"
        :args '("-m" "http.server" "3003")
        :cwd (expand-file-name "~/roam/gisual/public")
        :tags '(work autostart)
        :kill-signal 'sigkill))
...
#+end_src
**** Set Emacs + EXWM as the KDE window manager
#+begin_src sh :tangle ~/.config/plasma-workspace/env/exwm.sh
  export KDEWM=~/.doom.d/start-exwm.sh
#+end_src
**** Configure EXWM
...
#+end_src
** 1Password
A simple Emacs function to retrieve secrets from the 1Password CLI

#+begin_src emacs-lisp
  (defun op-read (secret-ref)
    (string-trim
     (with-temp-buffer
       (call-process "op" nil t nil "read" secret-ref)
       (buffer-string))))
#+end_src
